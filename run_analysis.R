...
final2<-final[,c(measurements, 562, 563)]
final2
...
final3<-merge(final2, activity, by.x = "X562", by.y = "V1")
...
features2<-features1[c(measurements)]
features2<-c(features2,newrows)
## I transform a little variable names

features_final<-gsub("-", "", features2)

features_final2<-gsub("()", "", features_final, fixed=TRUE)
...
final_final
write.table(final_final, "Assignment_MK.txt", row.names=FALSE)
